-- Migration de nettoyage et sécurisation - MeetRun (Corrigée)
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_max_participants_positive') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'check_host_payout_valid') THEN
    ALTER TABLE public.sessions 
