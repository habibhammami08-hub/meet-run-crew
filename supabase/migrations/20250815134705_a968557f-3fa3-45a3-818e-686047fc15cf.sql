-- =====================================================
-- MIGRATION FINALE DE CONSOLIDATION - MEETRUN V2
-- =====================================================
...
BEGIN
    ...
    -- Supprimer date et time obsolètes si elles existent
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'sessions' AND column_name = 'date') THEN
        ALTER TABLE public.sessions DROP COLUMN date CASCADE;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'sessions' AND column_name = 'time') THEN
