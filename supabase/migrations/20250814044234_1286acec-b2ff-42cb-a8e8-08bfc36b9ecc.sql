...
-- Create computed column for has_active_sub
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS has_active_sub BOOLEAN 
GENERATED ALWAYS AS (
  sub_status IN ('active', 'trialing') AND 
  (sub_current_period_end IS NULL OR sub_current_period_end > now())
) STORED;
...
SET sub_status = 'inactive', has_active_sub = false 
